  ...
  if (!products || products.length === 0) {
    return (
      <div className="bg-white border border-[#1B4332]/5 rounded-[2rem] py-32 flex flex-col items-center justify-center text-center px-6 shadow-xl">
         ...
         <h4 className="text-2xl font-black text-[#1B4332]/40 uppercase italic tracking-tighter">No Pending Products</h4>
         <p className="text-[10px] font-bold text-[#1B4332]/20 uppercase tracking-[0.3em] mt-3 max-w-xs mx-auto">All new products have been checked.</p>
      </div>
    );
  }
          ...
          <table className="w-full text-left border-collapse relative">
             <thead className="sticky top-0 z-20">
                <tr className="bg-white border-b border-[#1B4332]/10 shadow-[0_1px_0_rgba(27,67,50,0.05)]">
                   <th className="px-8 py-4 text-[9px] font-black uppercase tracking-[0.2em] text-[#1B4332]/30 bg-white">Product Info</th>
                   <th className="px-4 py-4 text-[9px] font-black uppercase tracking-[0.2em] text-[#1B4332]/30 text-center bg-white">Weight</th>
                   <th className="px-4 py-4 text-[9px] font-black uppercase tracking-[0.2em] text-[#1B4332]/30 text-center bg-white">Price</th>
                   <th className="px-4 py-4 text-[9px] font-black uppercase tracking-[0.2em] text-[#1B4332]/30 bg-white">Shop Name</th>
                   <th className="px-4 py-4 text-[9px] font-black uppercase tracking-[0.2em] text-[#1B4332]/30 text-center bg-white">Status</th>
                   <th className="px-8 py-4 text-[9px] font-black uppercase tracking-[0.2em] text-[#1B4332]/30 text-right bg-white">Action</th>
                </tr>
             </thead>
                  ...
                  <tr key={p.id || `prod-${idx}`} className="group hover:bg-[#FDFBF7] transition-colors">
                     ...
                     <td className="px-8 py-4">
                        <div className="space-y-0.5">
                           <p className="text-[13px] font-black text-[#1B4332] uppercase tracking-tight group-hover:text-[#BC6C25] transition-colors">{p.title || p.name}</p>
                     ...
                     <td className="px-4 py-4">
                        <div className="flex justify-center">
                           <div className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest flex items-center gap-1.5 border ${p.is_approved ? 'bg-emerald-500/5 border-emerald-500/10 text-emerald-600' : p.status === 'rejected' ? 'bg-rose-500/5 border-rose-500/10 text-rose-600' : 'bg-amber-500/5 border-amber-500/10 text-amber-600'}`}>
